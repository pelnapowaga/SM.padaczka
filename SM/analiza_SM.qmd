...
## Wykres gęstości wyników: pre-test/re-test + płeć + demografia - tu jest coś skopane
...
t.test(pivot_df_testA$wynik, pivot_df_testB$wynik,
       paired = TRUE,   
       var.equal = FALSE,
       conf.level = 0.95)
...
cohensD(pivot_df_plec_m$wynik, pivot_df_plec_k$wynik)
...
cohensD(pivot_df_D5b$wynik, pivot_df_D5i$wynik)
...
## Modele liniowe dla efektu prezentacji
...
mod1 <- lm(wiedza ~ D4, data = DF_SM_4) #miejsce zamieszkania
summary(mod1)
...
mod2 <- lm(wiedza ~ D9, data = DF_SM_4) #deklarowana wiedza
summary(mod2)
