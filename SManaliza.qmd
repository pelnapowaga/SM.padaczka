...
#macież korelacji
cor_answers_B<-cor(answers_B, method = "spearman")
View(cor_answers_B) 

sum(cor(cor_answers_B)<0.3)/length(cor_answers_B-14)*100 #80% korelacji jest poniżej 0.3
(sum(cor(cor_answers_B)>0.9)-14)/length(cor_answers_B-14)*100 #nie ma żadnej korelacji powyżej 0.9

#wyznacznik macierzy korelacji
det(cor(answers_B)) #0.02040104

#kryterium KMO
KMO(cor(answers_B)) 

#czy macierz jest macierzą identycznościową
cortest.bartlett(cor(answers_B)) #H0: macierz korelacji jest identycznościowa - dane nie mają ze sobą nic wspólnego; mamy podstawy do orzucenia H0

#właściwa EFA
fit_2<-fa(r = answers_B, nfactors = 1, rotate = "varimax")
print(fit_2)


#dla QA optymalne są dwa czynniki, a dla QB optymalny jest 1 czynnik
